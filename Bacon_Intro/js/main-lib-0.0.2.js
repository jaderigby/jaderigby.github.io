...
_$('#reveal').click((e) => {
	if (currFrame === 'first') {
			...
			else if (_$('#' + currFrame + ' [class$=-sequence] > div').items.length !== 0) {
				let items = _$("#" + currFrame + ' .show').items;
				let latest = 0;
				if (items.length > 0) {
					latest = items.length;
				}
				let showList = _$('#' + currFrame + ' [class$=-sequence] > div').items;
				let currShow = showList[latest];
				if (showList.length !== items.length) {
					show(currShow);
				}
...
window.addEventListener('keydown', (e) => {
	...
	if (e.key === r_key) {
		...
		if (e.shiftKey) {
			...
			else if ((e.key === d_key || e.key === enter_key) && _$("#" + currFrame + ' [class$=-sequence] p, #' + currFrame + ' [class$=-sequence] li, #' + currFrame + ' [class$=-sequence] > div').items.length !== 0) {
				let items = _$("#" + currFrame + ' .show').items;
				let latest = 0;
				if (items.length > 0) {
					latest = items.length - 1;
				}
				let showList = _$('#' + currFrame + ' [class$=-sequence] p, #' + currFrame + ' [class$=-sequence] li, #' + currFrame + ' [class$=-sequence] > div').items;
				let currShow = showList[latest];
				hide(currShow);
			}
			...
			else if ((e.key === d_key || e.key === enter_key) && _$('#' + currFrame + ' [class$=-sequence] > div').items.length !== 0) {
				let items = _$("#" + currFrame + ' .show').items;
				let latest = 0;
				if (items.length > 0) {
					latest = items.length;
